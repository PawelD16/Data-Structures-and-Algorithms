...
public class AdjacencyListWeightedDigraph implements IWeightedDigraph {
    ...
    public List<Converter.WeightedEdge> verticesConnectedToAsWeightedEdges(int v) {
        ...
        for (Vertex vertex : vertices) {
            if (vertex.numberOfFirstVertex == v) {
                for (Edge edge : vertex.edges) {
                    list.add(new Converter.WeightedEdge(v, edge.numberOfSecondVertex, edge.weight));
                }
            }
            if (vertex.numberOfFirstVertex > v) break;
        }
    ...
    public boolean containsVertex(int v) {
        for (Vertex vertex : vertices){
            if(vertex.numberOfFirstVertex == v) return true;
            if (vertex.numberOfFirstVertex > v) return false;
        }
        return false;
    }
