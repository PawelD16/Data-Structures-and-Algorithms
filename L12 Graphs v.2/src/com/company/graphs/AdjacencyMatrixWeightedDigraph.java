...
public class AdjacencyMatrixWeightedDigraph implements IWeightedDigraph {
    ...
    public List<Integer> verticesConnectedTo(int v) {
        ...
        if (v >= matrix.length) return list; //zwracamy pusta liste jezeli nie istnieje dany wezel

        for (int i = 0; i < matrix.length; ++i) {
            if (matrix[v][i] != Double.POSITIVE_INFINITY) list.add(i);
        }
    ...
    @Override
    public List<Converter.WeightedEdge> verticesConnectedToAsWeightedEdges(int v) {
        if (v < 0) throw new IllegalArgumentException();
        List<Converter.WeightedEdge> list = new ArrayList<>();
        if (v >= matrix.length) return list; //zwracamy pusta liste jezeli nie istnieje dany wezel

        for (int i = 0; i < matrix.length; ++i) {
            if (matrix[v][i] != Double.POSITIVE_INFINITY)
                list.add(new Converter.WeightedEdge(v, i, matrix[v][i]));
        }
    ...
    public boolean containsVertex(int v) {
        ...
        /*
        for (int i = 0; i < matrix.length; ++i) {
            if (matrix[i][v] != Double.POSITIVE_INFINITY || matrix[v][i] != Double.POSITIVE_INFINITY) return true;
        }
        return false;
         */
    }
